...
public class HomeCareFog2 {
	...
	static int lcloud = 4; // area to cloud
	static int lareas = 2; // device to area
	static double lsensor = 6; // sensor to device
...
//		}
	...
	private static MyFogDevice addEnd(String endPartialName, int userId, String appId, int parentId){
		...
		sensor.setLatency(lsensor);  // latency of connection between EEG sensors and the parent Smartphone is 6 ms
		actuator.setGatewayDeviceId(end.getId());
		actuator.setLatency(lactuat);  // latency of connection between Display actuator and the parent Smartphone is 1 ms
		return end;
	}
