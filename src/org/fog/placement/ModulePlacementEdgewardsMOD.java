...
public class ModulePlacementEdgewardsMOD extends ModulePlacement {
	...
	protected void mapModules() {
		...
		List<List<Integer>> leafToRootPaths = getLeafToRootPaths(); // montando os caminhos possíveis da árvore (folha
																	// para raíz)
	 ...
	 */
	private List<String> getModulesToPlace(List<String> placedModules) { // busca módulos predecessores com base no
...
//		System.out.println("moduleToPlace:"+modulesToPlace);
	...
	protected double getRateOfSensor(String sensorType) { // calculo da taxa de transmissão do sensor com base no valor
															// sensingInterval
	...
	private void placeModulesInPath(List<Integer> path) {
		...
		for (AppEdge edge : getApplication().getEdges()) { // calculando taxa de tupla caso haja periodicidade para
															// aresta
			if (edge.isPeriodic()) { // periodicidade definida na especificação da aplicação
...
//			System.out.println(path);
			...
			List<String> modulesToPlace = getModulesToPlace(placedModules); // artigo: linha 4 do pseudocódigo + chamada
