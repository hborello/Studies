...
public class ModulePlacementEdgewards2 extends ModulePlacement{
	...
	public ModulePlacementEdgewards2(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, 
		...
		System.out.println("\n******** fim do metodo ModulePlacementEdgewards ");

//		System.out.println("Map Modules: "+this.moduleMapping + "\n");
	}
	...
	protected void mapModules() {
		...
		System.out.println("\n************ inicio mapModules *************\n******* FIXED PLACEMENT: " + getCurrentModuleMap());
		
		
		List<List<Integer>> leafToRootPaths = getLeafToRootPaths();
		
		System.out.println("******* LEAF TO ROOT PATHS: " + leafToRootPaths + "\n");
		
		for(List<Integer> path : leafToRootPaths){
			placeModulesInPath(path);
		}
		
//		System.out.println("Current Module Map: " + getCurrentModuleMap());
...
//			System.out.println("Periodicity:"+edge.isPeriodic());
			if(edge.isPeriodic()){
...
//					System.out.println("Edge: "+ edge);
					...
					for(Pair<String, String> pair : map.keySet()){
						if(pair.getFirst().equals(edge.getTupleType())){
							...
							System.out.println("******* Rate:"+ outputRate+" --- "+appEdgeToRate.get(edge)+" * "+map.get(pair).getMeanRate());
						}
					}
				}
			}
...
//								System.out.println("Rate: "+rate+" / Edge Cpu Length: " + edge.getTupleCpuLength()
//								+ " - " + totalCpuLoad);
							}
						}
						
//						System.out.println("Total Cpu Load:" + totalCpuLoad + " + getCurrentCpuLoad:" + 
//								getCurrentCpuLoad().get(deviceId)+ " = " + (totalCpuLoad+getCurrentCpuLoad().get(deviceId)) +
//								" > Host Mips: "+device.getHost().getTotalMips());
//						
...
//							System.out.println("ModulePlacementEdgeward "+ "AppModule "+moduleName+" can be created on device "+device.getName());
					...
					System.out.println("Total Cpu Load:" + totalCpuLoad + " + getCurrentCpuLoad:" + 
							getCurrentCpuLoad().get(deviceId)+ " = " + (totalCpuLoad+getCurrentCpuLoad().get(deviceId)) +
							" > Host Mips: "+device.getHost().getTotalMips() + "(" + device.getId()+")");
