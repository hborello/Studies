...
public class ModulePlacementEdgewards extends ModulePlacement{
	...
	protected void mapModules() {
		...
		for(String deviceName : getModuleMapping().getModuleMapping().keySet()){
			for(String moduleName : getModuleMapping().getModuleMapping().get(deviceName)){
				...
				getCurrentModuleMap().get(deviceId).add(moduleName);				// guardando quais módulos já estão colocados
				getCurrentModuleLoadMap().get(deviceId).put(moduleName, 0.0);
				getCurrentModuleInstanceNum().get(deviceId).put(moduleName, 0);
			}
		}
		
		List<List<Integer>> leafToRootPaths = getLeafToRootPaths();					// montando os caminhos possíveis da árvore (folha para raíz)
		
		
		for(List<Integer> path : leafToRootPaths){									// chamando o método de colocação para cada caminho gerado
			placeModulesInPath(path);												// artigo: linha 1 do pseudocódigo do artigo
		}
	 ...
	 */
	private List<String> getModulesToPlace(List<String> placedModules){ // busca módulos predecessores com base no grafo direcionado especificado na montagem da aplicação
		...
		return modulesToPlace;	// retorna o módulo caso seus predecessores já estejam colocados
	}
	
	protected double getRateOfSensor(String sensorType){ // calculo da taxa de transmissão do sensor com base no valor sensingInterval
	...
	private void placeModulesInPath(List<Integer> path) {
		...
		List<String> placedModules = new ArrayList<String>();					// artigo: linha 2 do pseudocódigo
		 ...
		 */
		for(AppEdge edge : getApplication().getEdges()){						// calculando taxa de tupla caso haja periodicidade para aresta 
			if(edge.isPeriodic()){												// periodicidade definida na especificação da aplicação (createApplication/addAppEdge)
				appEdgeToRate.put(edge, 1/edge.getPeriodicity());
			}
		}
		
		for(Integer deviceId : path){											// artigo: linha 3 do pseudocódigo
...
//			System.out.println("******* EDGE TO RATE: "+appEdgeToRate);
			 ...
			 */
			List<String> modulesToPlace = getModulesToPlace(placedModules);			// artigo: linha 4 do pseudocódigo + chamada para linha 5 (getModulesToPlace)
			
			// Loop runs until all modules in modulesToPlace are deployed in the path
			while(modulesToPlace.size() > 0){ 										// artigo: equivalenta a linha 10 do pseudocódigo
...
//								System.out.println(edge.getSource()+"-"+edge.getDestination()+"-"+edge.getTupleCpuLength());
						...
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){
							...
							List<String> _placedOperators = shiftModuleNorth(moduleName, totalCpuLoad, deviceId, modulesToPlace); // artigo: linha 15
...
//					Helberth: Acrescentei parâmetros de memória para fazer o placement
...
//					if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips() && !memory){
					if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()){ // artigo: linha 26
