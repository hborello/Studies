...
public class MyModulePlacement extends MyPlacement {
	...
	public MyModulePlacement(List<MyFogDevice> fogDevices, List<MySensor> sensors, List<MyActuator> actuators,
		...
		System.out.println("Module to place: " + moduleToPlace.toString());

		this.deviceMipsInfo = new HashMap<Integer, Integer>();
		mapModules();
		
//		List<AppModule> app = new this.getDeviceToModuleMap().get(3);
		System.out.println("Map Modules: "+ super.getDeviceToModuleMap());
//		[3, 4, 5, 8, 11, 12, 15]
...
//				System.out.println("Module Name: " + deviceId + "-"+ moduleName.toString() + " - " + deviceName);
				if (!getDeviceToModuleMap().containsKey(deviceId)) {
					...
					System.out.println("App Module: " + appModule.getName());
...
//			System.out.println("Device name: "+device.getName()+" Parent device: " + device.getId());
			int deviceParent = -1; // device parent cloud -- eu mudei aqui
			...
			if (device.getLevel() == 1) {
				...
				for (int i = 0; i < keys.size() - 1; i++) {
					for (int j = 0; j < keys.size() - i - 1; j++) {
						System.out.println(childDeadline.get(keys.get(j)));
						System.out.println(childDeadline.get(keys.get(j + 1)));
				...
				for (int key : keys) {
					...
					if (currentMips + baseMipsOfPlacingModule + additionalMips < device.getMips()) {
						...
						System.out.println(appModule.getName() + " not placed!");
					}
				}
